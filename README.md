...
The tool returns immediately with a deterministic job id. When the command finishes, the extension injects one `<pi_context source="pi-background-bash" kind="background_bash_result">` message into the session and wakes the agent.
